...
// Golobal variable:
const int n        = 20; // Number of steps
const int npart    = 1; // Number of particles
  ...
  */
{
  ...
  cout << setiosflags (ios::scientific);
	...
	// Create a file .dat for RK4 error:
	string fname1   = "rk4_err.dat";
	ofstream fdata1 (fname1.c_str(), ios::out);
	fdata1 << setiosflags(ios::scientific);
	
	// Create a file .dat for Boris error:
	string fname2   = "boris_err.dat";
  ...
  double vperp; // perpendicular velocity respect to B
	...
	while (iter < nw)
	{
	  ...
	  xmoda = sqrt(x[0]*x[0] + x[1]*x[1] + x[2]*x[2]);
		...
		relx = fabs((sqrt(Y[0]*Y[0] + Y[1]*Y[1] + Y[2]*Y[2]) - xmoda)/xmoda);
		relv = fabs((sqrt(Y[3]*Y[3] + Y[4]*Y[4] + Y[5]*Y[5]) - vmoda)/vmoda);
	  fdata1 << h << " " << relx << " " << relv << endl;
	  iter *= 2;
	}
